...
class RepresentationsDataset(Dataset):
    ...
    def __init__(
        ...
        self.metadata = self.metadata[
            (self.metadata['use_slide'] == True) & (self.metadata['Cohort'] == "UCL")
    ...
    def _get_label_dict(self) -> Dict[str, int]:
        ...
        conditions_list = self.metadata['Condition_disease'].tolist()
        _LABEL_map = {"HC": 0, "Disease": 1}
    ...
    def get_pos_weight(self) -> torch.Tensor:
        ...
        pos_weight : Tensor ``(1,)`` float32
            ``n_HC / n_Disease`` — upweights Disease (label=1) when HC
            is the majority class, downweights it when Disease dominates.
            Matches the formula expected by ``nn.BCEWithLogitsLoss(pos_weight=...)``.
        """
        conditions = self.metadata['Condition_disease'].tolist()
        n_neg = conditions.count('HC')       # label 0
        n_pos = conditions.count('Disease')  # label 1
        if n_pos == 0:
            raise ValueError("No 'Disease' slides found in this split — cannot compute pos_weight.")
        print(f"Class counts — HC (neg): {n_neg}, Disease (pos): {n_pos}, pos_weight: {n_neg/n_pos:.3f}")
...
class DiscriminatorInference(Dataset):
    ...
    def __init__(
        ...
        self.metadata = pd.read_csv(csv_path)
        self.metadata = self.metadata[
            self.metadata['use_slide'] == True
            ].reset_index(drop=True)
        
        self.max_tiles = max_tiles
        self.representation_dir = representation_dir
      
        print(f"Loaded {len(self.metadata)} slides for inference")
        
        # Get list of slide IDs that have both embeddings and labels
        self.slide_list = self._get_slide_list()
        
        if len(self.slide_list) == 0:
            raise ValueError(f"No slides found in csv file: {csv_path} for inference")
    
    def _get_slide_list(self) -> List[str]:
        """Create a list of slide IDs."""
        samples_list = self.metadata['Slide'].apply(lambda x: os.path.basename(x).split('.')[0]).tolist()
        return list(samples_list)
