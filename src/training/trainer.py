...
from models.Discriminator import DiscriminatorNet, MammothNet
...
class Discriminator(L.LightningModule):
    def __init__(
        ...
        self.val_metrics = torchmetrics.MetricCollection(
            {
                "accuracy": torchmetrics.classification.Accuracy(**task_kwargs),
                "balanced_accuracy": torchmetrics.classification.Accuracy(**{**task_kwargs, "average": "weighted"}),
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        # get predicted label from prob, < 0.5 is class 0 and >= 0.5 is class 1
        pred_label = (prob >= 0.5).long() if self.n_classes == 1 else prob.argmax(dim=0)
...
class MammothTrainer(L.LightningModule):
    def __init__(
            self,
            input_dim: int = 2560,
            hidden_dim: int = 1280,
